...
public class DishSkillHandler extends Handler{
    ...
    public DishSkillHandler(SemanticResult result) {
        super(result);
    }
    ...
    private void syncDishData() {
        ...
        MessagesViewModel.getInstance().syncSpeakableData(speakableSyncData);
        MessagesViewModel.getInstance().putPersParam("uid", "");

        MessagesViewModel.getInstance().fakeAIUIResult(0,
