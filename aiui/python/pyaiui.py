...
class IAIUIAgent:
    ...
    def createAgent(params: str, listener):
        ...
        if not ischildof(listener, AIUIEventListener):
            raise Exception("listener must implement from AIUIEventListener")

        agent = IAIUIAgent(None)
        agent.listenerWrapper = EventCallback(listener)
        agent.AIUIListenerCallback = ctypes.CFUNCTYPE(None, ctypes.c_void_p, ctypes.c_void_p)(agent.listenerWrapper)
